...
public class controllerCaixa implements Initializable {
	...
	void acaoBTMFecharConta(ActionEvent event) throws IOException {
		...
		FXMLLoader novasMesas = new FXMLLoader(getClass().getResource("../TelasCaixaView/FXMLNota.fxml"));
		Parent parentNovasMesas = novasMesas.load();
		sceneNovasMesas = new Scene(parentNovasMesas, 293.0,
				230.0 + controllerMesas.getMesaSelecionada().getPedidos().size() * 30);
