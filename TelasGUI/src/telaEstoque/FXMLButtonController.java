...
public class FXMLButtonController implements Initializable {
	...
	public void gerarNovoAlimento(ActionEvent event) {
		try {
			boolean adicionado = estoquePrincipal.adicionarAlimentoEstoque(NomeDoAlimento.getText(), DataDeVencimento.getText(), CodigoDeBarra.getText(), Integer.parseInt(QteDoProduto.getText()));
			if(adicionado == true) {
				Alimento alimento = new Alimento(NomeDoAlimento.getText(), CodigoDeBarra.getText(), DataDeVencimento.getText(), Integer.parseInt(QteDoProduto.getText()));
	...
	private void adicionarAlimentoLista() {
		estoquePrincipal.adicionarAlimentoEstoque(NomeDoAlimento.getText(), DataDeVencimento.getText(), CodigoDeBarra.getText(), Integer.parseInt(QteDoProduto.getText()));
		alimentosList.getItems().add(new Alimento(NomeDoAlimento.getText(),DataDeVencimento.getText(),CodigoDeBarra.getText(),Integer.parseInt(QteDoProduto.getText())));
	...
	public void confirmarRetiradaDeUnidades(ActionEvent event) {
		...
		try {
			...
			String mensagemInformacao = String.format("O estoque do %s ficará abaixo do mínimo.",alimento.getNomeProduto());
	...
	public void atualizarTabela() {
		if(fieldRelMinEstoque.getText().isEmpty() && fieldRelDataVencimento.getText().isEmpty()) {
			...
			codigoAlimen.setCellValueFactory(new PropertyValueFactory<>("codigoBarra"));
			qteAlimen.setCellValueFactory(new PropertyValueFactory<>("qtdProduto"));
			ObservableList<Alimento> alimentoDados = FXCollections.observableList(estoquePrincipal.pertoDeVencer(LocalDate.parse(fieldRelDataVencimento.getText(),estoquePrincipal.getFormatador())));
			Tabela.setItems(alimentoDados);
		}
		else if(!fieldRelMinEstoque.getText().isEmpty() && fieldRelDataVencimento.getText().isEmpty()) {
			nomeAlimen.setCellValueFactory(new PropertyValueFactory<>("nomeProduto"));
			...
			codigoAlimen.setCellValueFactory(new PropertyValueFactory<>("codigoBarra"));
			qteAlimen.setCellValueFactory(new PropertyValueFactory<>("qtdProduto"));
			ObservableList<Alimento> alimentoDados = FXCollections.observableList(estoquePrincipal.estoqueEmBaixa(Integer.parseInt(fieldRelMinEstoque.getText())));
			Tabela.setItems(alimentoDados);
		}
	...
	public void btinterrogacao() {
		if (btAjuda.getLayoutX() < 500 && botaoGerarProduto.isVisible()) {
			...
			String mensagemInformacao = String.format("Cadastre cada dado do alimento e clique em salvar, logo em seguida esse novo alimento será cadastrado no estoque!");
			...
			String mensagemInformacao = String.format("Digite a data limite ou a quantidade mínima em estoque e clique em visualizar e você verá que a tabela atualizará os alimentos presentes no estoque. Clique em print para obter um relatório impresso. E clique em voltar para voltar para a tela principal do estoque.");
	...
	public void telaAdicionar(boolean estado, boolean botao, boolean alimentoEscolhido, boolean darBaixa, boolean detalharAlimento)	{
